...
public class EmbeddingServiceOptimized : IDisposable
{
    ...
    private object?[]? PrepareArguments(string text)
    {
        ...
        else if (_requestType != null)
        {
            ...
            if (req != null)
            {
                ...
                TrySetProperty(req, "Prompt", text);
                TrySetProperty(req, "Input", text);
                TrySetProperty(req, "Inputs", new[] { text });
                TrySetProperty(req, "Text", text);
            }
            _cachedArgs[0] = req;
        }
    ...
    {
        // If we know which endpoint works, try it first
    ...
    private async Task<float[]?> TryEndpoint(string path, string text)
    {
        ...
        var body = new { model = _modelName, prompt = text, input = text }; // include both common field names
    ...
    private static float[] JsonElementToFloatArray(JsonElement el)
    {
        ...
        // If we didn't fill the array completely, return a trimmed version
